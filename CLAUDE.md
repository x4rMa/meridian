...
2. **Top candidates**: `getTopCandidates({limit: 10})` — applies ALL hard filters (TVL, fee/TVL, volatility, organic, holders, mcap, bin step, launchpad allow/block, token age, cooldowns, base mints already in use, dev blocklist), optional indicator confirmation, **and** PVP-rival detection (default: warn; `blockPvpSymbols: true` → hard filter). **Token age is band-based**: `config.screening.tokenAgeBands` (array of `{name, minHours, maxHours, thresholds?, screeningProfile}`) — a pool passes the age gate if its token age falls inside ANY band, and the admitting band's `thresholds` shallow-merge over the base screening config for that band's full filter evaluation (so a "mature" band can demand higher TVL while relaxing organic). Empty `tokenAgeBands` ⇒ legacy single `minTokenAgeHours`/`maxTokenAgeHours` envelope. The matched `age_band` + `screening_profile` are stamped on the candidate and flow into the deploy `signal_snapshot` (→ `lessons.json` performance entries) so historical trades stay attributable to the policy that selected them. `analyze-performance --bucket token_age_hours --bins ...` validates the band hypothesis with real data.
...
| `screening` | `excludeHighSupplyConcentration`, `minFeeActiveTvlRatio`, `minTvl`, `maxTvl`, `minVolume`, `minOrganic`, `minQuoteOrganic`, `minHolders`, `minMcap`, `maxMcap`, `minBinStep`, `maxBinStep`, `timeframe`, `category`, `minTokenFeesSol`, `useDiscordSignals`, `discordSignalMode`, `avoidPvpSymbols`, `blockPvpSymbols`, `maxBotHoldersPct`, `maxTop10Pct`, `allowedLaunchpads`, `blockedLaunchpads`, `minTokenAgeHours`, `maxTokenAgeHours`, `tokenAgeBands` | see `user-config.example.json` |
