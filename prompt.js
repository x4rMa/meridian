...
export function buildSystemPrompt(agentType, portfolio, positions, stateSummary = null, lessons = null, perfSummary = null, weightsSummary = null, decisionSummary = null, markovSummary = null) {
...
${lessons ? `LESSONS LEARNED:\n${lessons}\n` : ""}${markovSummary ? `MARKOV PREDICTIONS:\n${markovSummary}\nUse state predictions to inform trailing take-profit adjustments. A high-confidence DOWNTREND prediction supports closing; a STABLE/UPTREND prediction supports holding.\n` : ""}Timestamp: ${new Date().toISOString()}
`;
  }
...
${markovSummary ? `MARKOV STABILITY:\n${markovSummary}\nDeprioritize pools with high Markov volatility (entropy > 60%) — their next state is unpredictable.\n\n` : ""}${weightsSummary ? `${weightsSummary}\nPrioritize candidates whose strongest attributes align with high-weight signals.\n\n` : ""}${lessons ? `LESSONS LEARNED:\n${lessons}\n` : ""}Timestamp: ${new Date().toISOString()}
`;
